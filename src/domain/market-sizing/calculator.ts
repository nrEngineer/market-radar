...
export function calculateProductivityAppMarket(): MarketSizingCalculation {
  return {
    ...
    cagr: {
      value: parseFloat(((1.132 * 1.102 - 1) * 100).toFixed(1)),
      period: '2024-2028',
      source: 'Fortune Business Insights「Productivity Management Software Market」Global CAGR 13.2%をベースに、日本AI市場の成長プレミアム（10.2%）を複利合成。計算: (1.132 × 1.102) - 1 = 24.7%',
      calculation: 'Compound: (1 + 0.132) × (1 + 0.102) - 1 = 24.7%',
    },
  }
}
...
export function calculateCreativeToolsMarket(): MarketSizingCalculation {
  return {
    ...
    cagr: {
      value: parseFloat(((1.174 * 1.138 - 1) * 100).toFixed(1)),
      period: '2024-2028',
      source: 'Grand View Research「AI Image Generator Market」Global CAGR 17.4%と日本クリエイターエコノミー成長プレミアム13.8%を複利合成',
      calculation: 'Compound: (1 + 0.174) × (1 + 0.138) - 1 = 33.6%',
    },
  }
}
